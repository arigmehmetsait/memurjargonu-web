...
interface BoslukDoldurmaCollection {
  id: string;
  name: string;
  soruSayisi: number;
}
...
interface BoslukDoldurmaDeneme {
  ...
  description: string;
  soruSayisi: number;
  collections?: BoslukDoldurmaCollection[];
  createdAt?: Date;
  updatedAt?: Date;
  status?: string;
}
...
export default function AdminBoslukDoldurmaPage() {
  ...
  const [collectionToEdit, setCollectionToEdit] =
    useState<BoslukDoldurmaCollection | null>(null);
  ...
  const handleSoruYonetimiClick = (deneme: BoslukDoldurmaDeneme) => {
      ...
      if (data.success) {
        ...
        if (updatedDenemeler && selectedDenemeForCollections) {
          const updatedDeneme = updatedDenemeler.find(
            (d: BoslukDoldurmaDeneme) =>
              d.id === selectedDenemeForCollections.id
          );
          if (updatedDeneme) {
  ...
  const handleEditCollectionClick = (collection: BoslukDoldurmaCollection) => {
  ...
  const handleDeleteCollectionClick = (
    collection: BoslukDoldurmaCollection
  ) => {
