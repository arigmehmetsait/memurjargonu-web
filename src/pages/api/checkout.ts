...
export default async function handler(
  ...
  try {
    ...
    if (!merchant_id || !merchant_key || !merchant_salt) {
      ...
      return res
        .status(500)
        .json({ error: "Payment provider configuration error" });
    }

    const merchant_oid =
      "ORD" + crypto.randomBytes(4).toString("hex") + Date.now();
    ...
    const merchant_fail_url =
      process.env.NEXT_PUBLIC_SITE_URL + "/payment/fail";
    ...
    const test_mode = 0; // 0 for production (canlı mod), 1 for test
    // Kullanıcı bilgilerini Firestore'dan al, yoksa varsayılan değerler kullan
    const user_name =
      userData?.displayName ||
      decoded.name ||
      email.split("@")[0] ||
      "Kullanıcı";
    const user_address = userData?.address || "Adres bilgisi girilmemiş";
    const user_phone = userData?.phone || decoded.phone_number || "0000000000";
    const debug_on = 0; // Canlı modda debug kapalı
    ...
    if (paytrData.status !== "success") {
      ...
      return res
        .status(400)
        .json({ error: paytrData.reason || "PayTR token error" });
    }
