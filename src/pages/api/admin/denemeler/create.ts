  ...
  try {
    ...
    if (existingDoc.exists) {
      return res.status(409).json({
        success: false,
        error: "Bu isimde bir deneme zaten mevcut",
      });
    }

    // Yeni deneme dokümanını oluştur
    const denemeRef = adminDb.collection("MevzuatDeneme").doc(denemeName);
    ...
    console.log(`Yeni deneme oluşturuldu: ${denemeName}`);
