...
export default async function handler(
  ...
  try {
    ...
    console.log(`Deneme silme işlemi başlatılıyor: ${denemeId}`);
    ...
    console.log(`Silinecek deneme: ${denemeData?.name || denemeId}`);
    ...
    sorularSnapshot.docs.forEach((soruDoc) => {
      ...
      console.log(`Soru silinecek: ${soruDoc.id}`);
    });

    // Deneme dokümanını sil
    batch.delete(denemeDoc.ref);
    console.log(`Deneme dokümanı silinecek: ${denemeId}`);

    // Batch işlemini uygula
    await batch.commit();

    console.log(`Deneme başarıyla silindi: ${denemeId}`);

    res.status(200).json({
      success: true,
      message: "Deneme başarıyla silindi",
      data: {
        denemeId,
    ...
    console.error("Deneme silme işlemi hatası:", error);
    res.status(500).json({
      success: false,
      error: "Deneme silme işlemi başarısız",
